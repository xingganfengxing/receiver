 ...
 * @author kk
 */
public abstract class AbstractBaseLiveMultipleServlet extends BaseServlet implements IBaseliveReceiver{
    
    private static final Logger LOG = LoggerFactory.getLogger(AbstractBaseLiveMultipleServlet.class);
    public static final int KAFKA_POOL_SIZE_FOR_LIVE_SERVER = StringUtils
            .isEmpty(Env.get("kafkaPoolSizeForLiveServer")) ? 10 : Integer.parseInt(Env
            .get("kafkaPoolSizeForLiveServer"));
    // public static final int KAFKA_POOL_SIZE_FOR_LIVE_RTMP_SERVER =
    // StringUtils.isEmpty(Env
    // .get("kafkaPoolSizeForLiveRtmpServer")) ? 1 :
    // Integer.parseInt(Env.get("kafkaPoolSizeForLiveRtmpServer"));
                ...
                Future<Boolean> future = daemonExecutor.submit(new Callable<Boolean>(){
                    public Boolean call() {
                        ...
                        try {
                            ...
                            // exceptionHandler.handleEventException(e, line,
                            // exceptionData, this.getClass());
...
//            }
    ...
    private void template(String line) throws Exception {
        ...
        if (_vflag) {
            ...
            // 判断日志时间是否为30分钟以内
            long time = 0L;
            String slogtime = paramMap.get("timestamp_parse");// 已完成格式化
            if (null == slogtime) {
                String slogtimeBak = paramMap.get("timestamp_bak");
                slogtime = slogtimeBak == null ? paramMap.get("timestamp") : slogtimeBak;
                time = threadLocal.get().parse(slogtime).getTime();
            } else {
                time = Long.parseLong(slogtime);
            }
