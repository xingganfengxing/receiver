 ...
 */
public class LiveDisplayServlet extends BaseServlet{
    private static final long serialVersionUID = 1L;
    private static final Logger LOG = LoggerFactory.getLogger(LiveDisplayServlet.class);
    ...
    protected void execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            // 条件：stream_id type
            String streamid = paramMap.get(LiveDisplayServlet.STREAMID); // 流ID
            String type = paramMap.get(LiveDisplayServlet.TYPE);// 粒度
            ...
            String valueM = liveDisplayService.query(startMysqlTime, endMysqlTime, streamid);
    ...
    private String getCacheData(String startCache, String streamId, MemcachedClient memcachedClient)
        ...
        for (int i = 1; i < 31; i++) {
            ...
            long visit = 0;
            long bw = 0;
            String keu_prefix = streamId + "_" + timestamp + "_" + "Q";
            String onlineKey = keu_prefix + "_online";
            String bwKey = keu_prefix + "_bw";
            Object ob = MemcacheUtil.get(onlineKey);
            Object ob1 = MemcacheUtil.get(bwKey);
            if (ob != null && ob1 != null) {
                Long online = Long.valueOf(ob.toString());
                Long bandwith = Long.valueOf(ob1.toString());
                visit += online.longValue();
                bw += bandwith.longValue();
            }
            sb.append(timestamp).append("\t").append(visit).append("\t").append(bw);
            sb.append("</br>");
        }
        return sb.toString();
    }
