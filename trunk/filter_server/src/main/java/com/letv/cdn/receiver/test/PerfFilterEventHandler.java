 ...
 * 复杂过滤业务测试
 */
public class PerfFilterEventHandler implements EventHandler<RequestInfo>{
    
    private static final Logger log = LoggerFactory.getLogger(PerfFilterEventHandler.class);
    private long count = TestA.ITERATIONS - 1;
    private CountDownLatch latch = TestA.latch;
    private final int ordinal; // 当前消费线程的编号
    
    public PerfFilterEventHandler() {
    
        this(0);
    }
    
    public PerfFilterEventHandler(Integer ordinal) {
    ...
    public void onEvent(RequestInfo requestInfo, long sequence, boolean endOfBatch) throws Exception {
         ...
         System.out.println("PerfFilterEventHandler: " + sequence);
//        // if ((sequence % 2) == id) {
...
//                // KafkaProcessManager.getProcessById(1).sendObjectKafka(join);
//                // log.info("send kafka data:" + join);
//            } catch (Exception ex) {
//                log.error("filter event err", ex);
//            }
//        }
        if (count == sequence) {
            latch.countDown();
